...
class CognitoUserPoolClientClass implements IAuthClient {
	...
	constructor(config) {
		let userPoolConfig: CognitoUserPoolParams;

		if (config.plugins && config.plugins.length > 0) {
			userPoolConfig = config.plugins.find(
				i => i.pluginName === 'CognitoPlugin'
			).pluginConfig;
		} else if (config.Auth2) {
			userPoolConfig = config.Auth2.plugins.find(
				i => i.pluginName === 'CognitoPlugin'
			).pluginConfig;
		} else {
			userPoolConfig = config;
		}
		this.clientParameters = {
			userPoolId: userPoolConfig.aws_user_pools_id.split('_')[1],
			userPoolWebClientId: userPoolConfig.aws_user_pools_web_client_id,
			region: userPoolConfig.aws_cognito_region,
		};
		this.endpoint = `https://cognito-idp.${userPoolConfig.aws_cognito_region}.amazonaws.com/`;
	...
	public signIn = async (signInParams: SignInParams) => {
		...
		const cognitoUser = await this.refreshAuthUser({
			searchParams: { accessToken: tokens.accessToken },
		});

		user.attributes.providerAttributes.userName = cognitoUser.Username;

		return {
			user,
			credentials: { tokens },
			authState,
			authNProvider: 'Cognito',
		};
	};

	public refreshAuthUser = async (
		refreshAuthUserParams: RefreshAuthUserParams
	) => {
		let options = this.formOptions(
			'GetUser',
			refreshAuthUserParams.headers,
			refreshAuthUserParams.options
		);

		options.body = JSON.stringify({
			AccessToken: refreshAuthUserParams.searchParams.accessToken,
		});
